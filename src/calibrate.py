...
def main():
    ...
    y=torch.tensor(labels,dtype=torch.long); x=torch.tensor(logits,dtype=torch.float32)
    # Optimize log(T) so LBFGS cannot cross zero and hit a clamp with zero gradient.
    log_t=nn.Parameter(torch.zeros(1)); opt=torch.optim.LBFGS([log_t],lr=.1,max_iter=100)
    def closure():
        opt.zero_grad()
        temperature=torch.exp(log_t)
        loss=nn.functional.cross_entropy(x/temperature,y)
        loss.backward()
        return loss
    opt.step(closure)
    temperature=float(torch.exp(log_t.detach()).clamp(.05,10.0))
    probs=torch.softmax(x/temperature,dim=1).numpy(); args.output.parent.mkdir(parents=True,exist_ok=True); np.savez_compressed(args.output.with_suffix(".npz"),logits=logits,labels=labels,probs=probs); args.output.write_text(json.dumps({"temperature":temperature,"arch":arch,"num_classes":nc,"rows":len(df)},indent=2),encoding="utf-8"); print(json.dumps({"temperature":temperature,"rows":len(df)},indent=2))
