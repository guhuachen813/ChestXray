...
def main() -> None:
    ...
    checkpoint = torch.load(args.checkpoint, map_location="cpu"); num_classes = int(checkpoint.get("num_classes", 1)); frame = pd.read_csv(args.manifest); frame = frame[frame["Cardiomegaly"].isin([0, 1]) & frame.internal_split.eq(args.split)].copy(); device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = make_model(num_classes).to(device); model.load_state_dict(checkpoint["model"]); loader = DataLoader(CheXpertDataset(frame, args.data_root, args.image_size, False, num_classes), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    probs, labels = predict(model, loader, device, num_classes); threshold = args.threshold if args.threshold is not None else float(checkpoint.get("threshold", 0.5)); pred = (probs >= threshold).astype(int); tn, fp, fn, tp = confusion_matrix(labels, pred, labels=[0, 1]).ravel()
